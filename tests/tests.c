...
{
    char * ip = "127.0.0.1";
    unsigned max_len = 1024;
    char actual[max_len];
    actual[0] = '\0';
    max_len = geo_lookup_weather(get_handle(), ip, 1, actual, max_len);
...
void test_LookupCity(void)
{
    ...
    char actual[max_len];
    actual[0] = '\0';
    max_len = geo_lookup(get_handle(), ip, lookup_path, actual, max_len);
...
void test_LookupLocation(void)
{
    ...
    char actual[max_len];
    actual[0] = '\0';
    max_len = geo_lookup_location(get_handle(), ip, 1, actual, max_len);
    char *expected = "{\"city\":\"Beverly Hills\",\"state\":\"CA\",\"country\":\"US\"}";
    TEST_ASSERT_EQUAL_STRING(expected, actual);
}

void test_LookupTimezone(void)
{
    char *ip = "170.149.100.69";
    unsigned max_len = 1024;
    char actual[max_len];
    actual[0] = '\0';
    max_len = geo_lookup_timezone(get_handle(), ip, 1, actual, max_len);
    char *expected = "{\"timezone\":\"America/New_York\"}";
    TEST_ASSERT_EQUAL_STRING(expected, actual);
}
...
void test_GetWeatherCode(void)
{
    ...
    char actual[max_len];
    actual[0] = '\0';
    get_weather_code_from_cookie(cookiestra, cookiename, actual, max_len);
    TEST_ASSERT_EQUAL_STRING(expected, actual);
    actual[0] = '\0';
    ...
    actual[0] = '\0';
    get_weather_code_from_cookie(cookiestrb, cookiename, actual, max_len);
    TEST_ASSERT_EQUAL_STRING(expected, actual);
    actual[0] = '\0';
    ...
    get_weather_code_from_cookie(cookiestrc, cookiename, actual, max_len);
    TEST_ASSERT_EQUAL_STRING(expected, actual);
    ...
    actual[0] = '\0';
    max_len = get_weather_code_from_cookie(cookiestrd, cookiename, actual, max_len);
    TEST_ASSERT_EQUAL_STRING(expected, actual);
}
...
void test_GetEmptyCookieE()
{
    ...
    const char* cookiename = "NYT_W2";
    const char* expected="IndianapolisINUSÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿÿ|ChicagoILUS|London--UK|Los%20AngelesCAUS|San%20FranciscoCAUS|Tokyo--JP||";
    unsigned max_len = 1024;
    char actual[max_len];
    actual[0] = '\0';
    max_len = get_cookie(cookiestra, cookiename, actual, max_len);
...
void test_Strncpy()
{
    ...
    fprintf(stdout, "length: %d str: %s\n", l, str);
    strncpy(p, str, 4);
    fprintf(stdout, "p is [%s]\n", p);
    if (p[4] == '\0') {
        fprintf(stdout, "p[4] is null\n");
    } else {
        fprintf(stdout, "p[4] is not null\n");
    }
}
